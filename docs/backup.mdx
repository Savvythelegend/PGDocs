...
[_Troubleshooting / Emergency backup_ section](troubleshooting.mdx#emergency-backup)
for guidance.
:::
...
- **Natively**, with support for:
  - [Object storage via Barman Cloud](appendixes/backup_barmanobjectstore.mdx)
    _(although deprecated from 1.26 in favor of the Barman Cloud Plugin)_
  - [Kubernetes Volume Snapshots](appendixes/backup_volumesnapshot.mdx), if
...
This use case can also be extended to [replica clusters](replica_cluster.mdx),
...
When you [configure a WAL archive](wal_archiving.mdx), CloudNativePG provides
out-of-the-box an [RPO](before_you_start.mdx#rpo) ≤ 5 minutes for disaster
...
- **Object store–based backups**, via the [**Barman Cloud
  ...
  [**deprecated native integration**](appendixes/backup_barmanobjectstore.mdx)
- [**Volume Snapshots**](appendixes/backup_volumesnapshot.mdx), using the
...
- **Database size**: For very large databases (VLDBs), **volume snapshots are
  ...
  [Recovery Time Objective (RTO)](before_you_start.mdx#rto).
...
[`ScheduledBackupSpec`](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-ScheduledBackupSpec)
...
_six-field cron expression_ that includes seconds. This format follows the
...
[`BackupSpec`](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-BackupSpec) in the
API reference.
:::
...
- `volumeSnapshot` – Uses native [Kubernetes volume snapshots](appendixes/backup_volumesnapshot.mdx#how-to-configure-volume-snapshot-backups)
- `barmanObjectStore` – Uses [Barman Cloud for object storage](appendixes/backup_barmanobjectstore.mdx)
  _(deprecated starting with v1.26 in favor of the
  [Barman Cloud Plugin](https://cloudnative-pg.io/plugin-barman-cloud/),
  but still the default for backward compatibility)_
