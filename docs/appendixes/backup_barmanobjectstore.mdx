...
be _tarballs_. Both base backups and WAL files can be compressed
and encrypted.
...
on a [standby](../backup.mdx#backup-from-a-standby).

## Common object stores

If you are looking for a specific object store such as
[AWS S3](object_stores.mdx#aws-s3),
[Microsoft Azure Blob Storage](object_stores.mdx#azure-blob-storage),
[Google Cloud Storage](object_stores.mdx#google-cloud-storage), or a compatible
provider, please refer to [Appendix C - Common object stores for backups](object_stores.mdx).

## WAL archiving

WAL archiving is the process that feeds a [WAL archive](../backup.mdx#wal-archive)
in CloudNativePG.
...
your Recovery Point Objective ([RPO](../before_you_start.mdx#rpo)). Even though you change the value
...
_Point of Recoverability_ (`PoR`), a moving point in time determined by
`current time - recovery window`. The _first valid backup_ is the first
...
than the first valid backup will be marked as _obsolete_ and permanently
...
as documented in ["Configure the application database"](../recovery.mdx#configure-the-application-database).
