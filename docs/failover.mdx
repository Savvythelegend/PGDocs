...
1. It will first try a PostgreSQL's _fast shutdown_ with
...
2. If the fast shutdown fails, or its timeout is exceeded, a PostgreSQL's
   _immediate shutdown_ is initiated.
...
Failover may result in the service being impacted ([RTO](before_you_start.mdx#rto))
and/or data being lost ([RPO](before_you_start.mdx#rpo)):
...
:::warning
As already mentioned in the ["Instance Manager" section](instance_manager.mdx)
