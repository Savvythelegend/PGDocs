...
[Appendix B – Recovery from an Object Store](appendixes/backup_barmanobjectstore.mdx#recovery-from-an-object-store).
...
[volume snapshot backups](appendixes/backup_volumesnapshot.mdx).
...
:::warning
If you added or removed a [tablespace](tablespaces.mdx) in your cluster
...
See [Bootstrap an empty cluster](bootstrap.mdx#bootstrap-an-empty-cluster-initdb)
...
You can use the data uploaded to the object storage to _bootstrap_ a new
...
[Bootstrap from a backup](bootstrap.mdx#bootstrap-from-a-backup-recovery).
...
Barman Cloud plugin pointing to a _backup_ object store resource. This enables
...
Avoid reusing the same `ObjectStore` configuration for both _backup_ and
_recovery_ in the same cluster. If you must, ensure that each cluster uses a
