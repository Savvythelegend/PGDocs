...
- **In-Place updates of the operator**: introduce an in-place online update of the
  ...
  [documentation for more detailed information](../installation_upgrade.mdx#in-place-updates-of-the-instance-manager))
...
_This release is to celebrate the launch of
...
defining a specific image name._
...
_Version 1.9.0 is not available on OpenShift due to delays with the
release process and the subsequent release of version 1.9.1._
...
- Replica mode (**EXPERIMENTAL**): allow a cluster to be created as a replica
  of a source cluster. A replica cluster has a _designated primary_ and any
  number of standbys.
...
- Definition of the _read-write_ service to connect your applications to the
  only primary server of the cluster
- Definition of the _read_ service to connect your applications to any of the
