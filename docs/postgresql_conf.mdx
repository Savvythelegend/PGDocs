...
depending on the server version, based on the expected and observed workloads.
    ...
    ["Replication" section](replication.mdx#replication-slots-for-high-availability).)
...
The [`STANDBY_TCP_USER_TIMEOUT` operator configuration setting](operator_conf.mdx#available-options),
...
For further information, please refer to the ["Logging" section](logging.mdx).
...
:::important
With the introduction of [declarative extensions](declarative_database_management.mdx#managing-extensions-in-a-database)
...
[PGAudit](https://www.pgaudit.org/) on PostgreSQL clusters. For further information, please refer to the ["PGAudit" logs section.](logging.mdx#pgaudit-logs)
...
the operator automatically mounts a _memory-bound `EmptyDir` volume_ called `shm`
