...
["Replica clusters"](replica_cluster.mdx) section.
...
in contrast with _storage-level replication_.
...
_hot standby_. In 2011, PostgreSQL 9.1 brought synchronous replication at the
transaction level, supporting [RPO](before_you_start.mdx#rpo)=0 clusters. Cascading
replication was added in PostgreSQL 9.2 (2012). The foundations for
[logical replication](logical_replication.mdx) were established in PostgreSQL
...
to the ["Certificates" section](certificates.mdx#client-streaming_replica-certificate)
...
[`.spec.postgresql.synchronous` stanza](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-SynchronousReplicaConfiguration).
...
:::important
Make sure you have a clear understanding of what _ready/available_ means
...
the ["Postgres instance manager" section](instance_manager.mdx).
...
the _method `ANY` specifies a quorum-based synchronous replication and makes
transaction commits wait until their WAL records are replicated to at least the
requested number of synchronous standbys in the list_.
...
please check the ["Scheduling" section](scheduling.mdx).
...
(see ["Synchronous Replication (Deprecated)"](replication.mdx#synchronous-replication-deprecated)).
...
(see ["Synchronous replication"](replication.mdx#synchronous-replication)).
...
["replicationSlots" section in the API reference](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-ReplicationSlotsConfiguration).
...
:::info ""
Please refer to the ["Monitoring" section](monitoring.mdx) for details on
