...
["Importing Postgres databases"](database_import.mdx) section.
...
For more details, refer to the [Replica Cluster section](replica_cluster.mdx).
:::
...
Please see ["Recovery from Volume Snapshot objects"](recovery.mdx#recovery-from-volumesnapshot-objects)
for details.
:::

## The `bootstrap` section

The _bootstrap_ method can be defined in the `bootstrap` section of the cluster
...
clusters.
Refer to the [replica cluster section](replica_cluster.mdx) for more information.
...
the dedicated ["Recovery" section](recovery.mdx) for guidance on each method.

:::info "API reference"
Please refer to the ["API reference for the `bootstrap` section](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-BootstrapConfiguration)
...
access to one or more PostgreSQL clusters as _sources_.
The primary use cases include:

1. **Importing Databases:** Specify an external source to be utilized during
   the [importation of databases](database_import.mdx) via logical backup and
...
:::info "API reference"
Please refer to the ["API reference for the `externalClusters` section](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-ExternalCluster)
for more information.
:::
...
Thanks to the _convention over configuration paradigm_, you can let the
...
["Declarative database role management"](declarative_role_management.mdx).
:::
...
definition using a _defaulting webhook_.
...
executed as the _superuser_ (`postgres`) against three different databases, in
this specific order:
...
physical _base backup_ (`recovery`).
...
to the dedicated ["Recovery" section](recovery.mdx) for detailed guidance on
each method.

### Bootstrap from a live cluster (`pg_basebackup`)

The `pg_basebackup` bootstrap mode allows you to create a new cluster
(_target_) as an exact physical copy of an existing and **binary-compatible**
PostgreSQL instance (_source_) managed by CloudNativePG, using a valid
_streaming replication_ connection. The source instance can either be a primary
...
instance, thereby creating a _snapshot_. Once the cloning process has finished,
...
- source must be configured with enough `max_wal_senders` to grant
  ...
  one _walsender_ for the backup plus one for WAL streaming
