...
:::important
Based on the [Operator Capability Levels model](operator_capability_levels.mdx),
...
[image catalogs](image_catalog.mdx) by simply referencing the PostgreSQL
...
The ["Benchmarking"](benchmarking.mdx) section of the documentation provides
...
By default, CloudNativePG creates three Kubernetes [services](service_management.mdx)
...
[declarative database management](declarative_database_management.mdx) using
...
[management of PostgreSQL roles, users, and groups through declarative configuration](declarative_role_management.mdx)
...
CloudNativePG supports the installation of clusters with the [PostGIS](postgis.mdx)
...
clients, using either the _simple bind_ or _search+bind_ mode, as described in
...
Additionally, CloudNativePG supports [in-place updates of the instance manager](installation_upgrade.mdx#in-place-updates-of-the-instance-manager)
...
_Disaster recovery_ is a business continuity component that requires
that both backup and recovery of a database work correctly. While as a
starting point, the goal is to achieve [RPO](before_you_start.mdx#rpo) < 5
minutes, the long-term goal is to implement RPO=0 backup solutions. _High
availability_ is the other important component of business continuity. Through
...
the [RPO](before_you_start.mdx#rpo) of the database. This allows resources to
...
Achieve _zero data loss_ (RPO=0) in your local high-availability CloudNativePG
...
minimize the Recovery Time Objective ([RTO](before_you_start.mdx#rto)) in the
...
define [distributed database topologies](replica_cluster.mdx#distributed-topology)
...
refer to the [Postgres instance manager](instance_manager.mdx) documentation.
...
CloudNativePG supports [hibernation of a running PostgreSQL cluster](declarative_hibernation.mdx)
...
[PgBouncer](connection_pooling.mdx), one of the most popular open source
...
Capability level 4 is about _observability_: monitoring,
