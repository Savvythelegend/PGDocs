...
"[_Cloud Native_](https://github.com/cncf/toc/blob/main/DEFINITION.mdx)".
...
Please read the ["Architecture: Synchronizing the state"](architecture.mdx#synchronizing-the-state)
section.
...
["Custom Pod Controller"](controller.mdx) section for details and reasons behind
this decision.
...
through the relevant Kubernetes services. Moreover, the [instance manager](instance_manager.mdx),
...
- control High Availability of a Postgres cluster primarily via automated
  failover and switchover, coordinating itself with the [instance manager](instance_manager.mdx)
...
- extensibility, with extensions like [PostGIS](postgis.mdx) for geographical
  databases
...
As covered in the ["Architecture"](architecture.mdx) section, the main
...
synchronous replication in the cluster, introducing [RPO](before_you_start.mdx#rpo)=0
...
through the [replica cluster](replica_cluster.mdx) feature, providing disaster
...
Please look at ["Fencing"](fencing.mdx) or ["Hibernation"](declarative_hibernation.mdx).
...
["Declarative role management"](declarative_role_management.mdx) feature
