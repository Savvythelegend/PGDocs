...
Please refer to the ["Installation"](installation_upgrade.mdx) section and then proceed
...
to the ["API Reference" section](cloudnative-pg.v1.mdx).
:::
...
Please refer to the ["Examples" section](samples.mdx).
:::
...
install the _Kube Prometheus stack_ with our sample configuration
...
- From the Prometheus installation, you will have the Prometheus Operator
  watching for **any** `PodMonitor` (see [_monitoring_](monitoring.mdx)).
...
Please refer to the [monitoring section](monitoring.mdx) for more information.

![local prometheus](/img/prometheus-local.png)

You can also monitor the CloudNativePG operator by creating a PodMonitor to
target it. See the relevant section in the [monitoring page](monitoring.mdx#monitoring-the-operator-with-prometheus).
...
In the Prometheus console, you can click on the _Alerts_ menu to see the alerts
we just installed.
