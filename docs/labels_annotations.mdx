...
can be linked together and put in relationship through _labels_ and
_annotations_.
...
See [AppArmor](security.mdx#restricting-pod-access-using-apparmor)
for details.
...
segments from the dump. See [PostgreSQL core dumps](troubleshooting.mdx#postgresql-core-dumps)
...
`cnpg.io/hibernation`
: Applied to a `Cluster` resource to control the [declarative hibernation feature](declarative_hibernation.mdx).
...
instructions provided in [Operator configuration](operator_conf.mdx).
