...
- _storage-level replication_, normally persistent volumes
- _application-level replication_, in this specific case PostgreSQL
...
Replicas are usually called _standby servers_ and can also be used for
read-only workloads, thanks to the _Hot Standby_ feature.
...
other operations,
[scheduling](scheduling.mdx) the workloads in a declarative manner (based on
...
[PostgreSQL replica clusters](replica_cluster.mdx) in different regions and
...
the [replica cluster feature](replica_cluster.mdx)).
...
distributed topology hosting "passive" [PostgreSQL replica clusters](replica_cluster.mdx).
As with other Kubernetes workloads in this scenario, promotion of a Kubernetes
cluster as primary must be done manually.

Through the [replica cluster feature](replica_cluster.mdx), you can define a
...
please refer to the ["Service Management" section](service_management.mdx)
below.
:::
...
:::seealso "Replication"
Please refer to the ["Replication" section](replication.mdx) for more
...
Please refer to the ["Connecting from an application" section](applications.mdx) for
...
Please refer to the ["Connection Pooling" section](connection_pooling.mdx) for
...
_current primary_ by the Kubernetes operator, as depicted in the following
diagram:
...
- reduce global **recovery point objectives** ([RPO](before_you_start.mdx#rpo))
  by storing PostgreSQL backup data in multiple locations, regions and possibly
  using different providers (Disaster Recovery)
- reduce global **recovery time objectives** ([RTO](before_you_start.mdx#rto))
...
Please refer to the ["Replica Clusters" section](replica_cluster.mdx) for
