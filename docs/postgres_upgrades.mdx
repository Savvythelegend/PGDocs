...
CloudNativePG will trigger a [rolling update of the cluster](rolling_update.mdx),
...
1. [Logical dump/restore](database_import.mdx) – Blue/green deployment, offline.
2. [Native logical replication](logical_replication.mdx#example-of-live-migration-and-major-postgres-upgrade-with-logical-replication) – Blue/green deployment, online.
...
- By updating the major version in the image tag via the `.spec.imageName`
  option.
- Using an [image catalog](image_catalog.mdx) to manage version changes.

For details on supported image tags, see
["Image Tag Requirements"](container_images.mdx#image-tag-requirements).
...
The [extensions management feature](declarative_database_management.mdx#managing-extensions-in-a-database)
