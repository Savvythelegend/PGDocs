...
sets (also known as _replication sets_) generated from one or more tables within
a database. Publications can be defined on any PostgreSQL 10+ instance acting
as the _publisher_, including instances managed by popular DBaaS solutions in the
...
or leverage the [`cnpg publication create` plugin command](kubectl-plugin.mdx#logical-replication-publications).
...
:::info
Please refer to the [API reference](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-Publication)
...
Additionally, refer to the [CloudNativePG API reference](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-PublicationTarget)
...
can be created on any supported PostgreSQL instance acting as the _subscriber_.
...
:::info
Please refer to the [API reference](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-Subscription)
...
`app` database of the `freddie` cluster (_publisher_) to the `app` database of
the `king` cluster (_subscriber_). Here's an example of a `Subscription`
manifest:
...
["Bootstrap" section](bootstrap.mdx#the-externalclusters-section) of the
documentation.
...
You can use the [`cnpg` plugin](kubectl-plugin.mdx#synchronizing-sequences)
