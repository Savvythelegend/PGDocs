...
The CloudNativePG repository has the _"Private vulnerability reporting"_ option
...
- **[Software Bill of Materials
  ...
  [in-toto SPDX predicate standard](https://github.com/in-toto/attestation/blob/main/spec/predicates/spdx.mdx).
...
enables the instance manager (which is the _PID 1_ process of the container
...
CloudNativePG does not require _privileged_ mode for container execution.
The PostgreSQL containers run as the `postgres` system user. No component
whatsoever requires running as `root`.

Likewise, Volume access does not require _privileged_ mode nor `root`
...
You can find more information in the [networking document](networking.mdx).
...
See the ["Secrets" section in the "Connecting from an application" page](applications.mdx#secrets) for more information.
...
["Client TLS/SSL Connections"](ssl_connections.mdx#"Client TLS/SSL Connections") page
...
["PostgreSQL Configuration" page](postgresql_conf.mdx#the-pg_hba-section) of the documentation.
...
["PostgreSQL Configuration" page](postgresql_conf.mdx#the-pg_ident-section) of the documentation.
