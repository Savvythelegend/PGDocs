...
:::info ""
For details, see [`PgBouncerSpec`](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-PgBouncerSpec)
...
Finally, as a _superuser_ connect in each application database, and then create
...
[`PoolerSpec`](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-PoolerSpec) in the API reference.
...
[How to inspect the exported metrics](monitoring.mdx/#how-to-inspect-the-exported-metrics).
...
[`cnpg` plugin](kubectl-plugin.mdx#promote), and then restoring the `paused`
attribute to `false`.
