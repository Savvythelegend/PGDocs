...
1. **Disaster Recovery and High Availability**: Enhance disaster recovery and,
   ...
   terms, this is known as a ["Distributed Topology"](replica_cluster.mdx#distributed-topology).
...
2. **Read-Only Workloads**: Create standalone replicas of a PostgreSQL cluster
   ...
   is referred to as a ["Standalone Replica Cluster"](replica_cluster.mdx#standalone-replica-clusters).

For example, the diagram below — taken from the ["Architecture" section](architecture.mdx#deployments-across-kubernetes-clusters)
...
This is normally the case of the ["Standalone Replica Cluster"](replica_cluster.mdx#standalone-replica-clusters).
...
- any role defined using the [declarative interface](declarative_role_management.mdx)
...
`Cluster`.
This is normally the case of the ["Distributed Topology"](replica_cluster.mdx#distributed-topology).
...
["Bootstrap" section](bootstrap.mdx#bootstrap-from-another-cluster).
...
- **`plugin` section**:
  - Enables bootstrapping the replica cluster using a [CNPG-I](https://github.com/cloudnative-pg/cnpg-i)
    ...
    [`restore_job`](https://github.com/cloudnative-pg/cnpg-i/blob/main/docs/protocol.mdx#restore_job-proto)
    and the [`wal`](https://github.com/cloudnative-pg/cnpg-i/blob/main/docs/protocol.mdx#wal-proto) protocols.
...
[“Configuring replication”](replica_cluster.mdx#defining-an-external-cluster)
section.
...
2. Set the `.spec.replica.promotionToken` with the value obtained from the
   former primary cluster (refer to ["Demoting a Primary to a Replica Cluster"](replica_cluster.mdx#demoting-a-primary-to-a-replica-cluster)).
...
uses different names, you must specify them as documented in [Configure the application database](bootstrap.mdx#configure-the-application-database).
...
The previous configuration assumes that the application database and its owning
user are set to the default, `app`. If the PostgreSQL cluster being restored
uses different names, you must specify them as documented in [Configure the application database](recovery.mdx#configure-the-application-database).
You should also consider copying over the application user secret from
the original cluster and keep it synchronized with the source.
See ["About PostgreSQL Roles"](#about-postgresql-roles) for more details.
...
application database](recovery.mdx#configure-the-application-database).
You should also consider copying over the application user secret from
the original cluster and keep it synchronized with the source.
See ["About PostgreSQL Roles"](#about-postgresql-roles) for more details.
...
[`.spec.replica.minApplyDelay` option](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-ReplicaClusterConfiguration),
