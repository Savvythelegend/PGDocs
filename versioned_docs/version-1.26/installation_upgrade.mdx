...
Please refer to ["`cnpg` plugin"](./kubectl-plugin.mdx#generation-of-installation-manifests) documentation
...
```sh
curl -sSfL \
  https://raw.githubusercontent.com/cloudnative-pg/artifacts/main/manifests/operator-manifest.yaml | \ kubectl apply --server-side -f - ```
|  |
...
```sh
curl -sSfL \
  https://raw.githubusercontent.com/cloudnative-pg/artifacts/release-1.26/manifests/operator-manifest.yaml | \ kubectl apply --server-side -f - ```
|  |
...
`fullnameOverride` field in the [_"values.yaml"_ file](https://helm.sh/docs/chart_template_guide/values_files/).
:::
...
["Operator configuration"](operator_conf.mdx) section.
:::

## Upgrades

:::important
Please carefully read the [release notes](release_notes.mdx)
...
This process is discussed in-depth on the [Rolling Updates](rolling_update.mdx) page.
:::
...
CloudNativePG provides two configuration options at the [operator level](operator_conf.mdx)
...
[operator configuration](operator_conf.mdx#available-options).
...
The [release notes](release_notes.mdx) page contains a detailed list of the
...
to a [declarative approach for cluster hibernation](declarative_hibernation.mdx).
...
the ["Distributed Topology" section for replica clusters](replica_cluster.mdx#distributed-topology).
