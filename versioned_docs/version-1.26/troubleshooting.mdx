...
- as much as you can about the actual [storage](storage.mdx), including storage
...
- [`cnpg` plugin](kubectl-plugin.mdx) for `kubectl`
...
1. the [status subcommand](kubectl-plugin.mdx#status) provides an overview of a
   cluster
2. the [report subcommand](kubectl-plugin.mdx#report) provides the manifests
...
Please refer to the [plugin document](kubectl-plugin.mdx) for complete instructions.
...
accordance with Kubernetes conventions, using [JSON format](logging.mdx).
...
- Use the [`kubectl cnpg logs` command](kubectl-plugin.mdx#logs) for
...
(see the ["Details about the deployment" section](installation_upgrade.mdx#details-about-the-deployment)
for details).
...
```shell
kubectl logs -n cnpg-system \
  deployment/cnpg-controller-manager --all-containers=true | \ jq -r . > cnpg_logs.json ```
|  |
...
_ready_ state, and with `<CLUSTER>-1` being the primary.
...
useful if you are using [affinity/anti-affinity rules and/or tolerations](scheduling.mdx).
...
You can find more information in the [networking](networking.mdx) section.
...
:::info
Please refer to ["Labels and annotations"](labels_annotations.mdx)
...
Please look at the ["Volume expansion" section](storage.mdx#volume-expansion) in the documentation.
...
the ["Disk Full Failure" section](instance_manager.mdx#disk-full-failure)
...
["The `postgresql` section"](postgresql_conf.mdx#the-postgresql-section)), and
...
In the [networking page](networking.mdx) you can find a network policy file
...
[operator configuration](operator_conf.mdx#available-options). This will cause
