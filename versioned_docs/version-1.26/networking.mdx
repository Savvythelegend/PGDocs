...
increased security.
As mentioned in the [security document](security.mdx), please ensure the operator can reach every cluster pod on ports 8000 and 5432, and that pods can connect to each other.
...
The operator _must_ be able to connect to cluster pods.
...
While [bootstrapping](bootstrap.mdx) from another cluster or when using the `externalClusters` section,
