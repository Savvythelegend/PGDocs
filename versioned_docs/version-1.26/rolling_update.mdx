...
- the [image catalog](image_catalog.mdx) is updated with a new image for the major used by the cluster;
...
- after the operator is updated, to ensure the Pods run the latest instance
  manager (unless [in-place updates are enabled](installation_upgrade.mdx#in-place-updates-of-the-instance-manager)).
...
requirements in terms of [RPO](before_you_start.mdx#rpo) and
[RTO](before_you_start.mdx#rto), whether your PostgreSQL architecture is shared
...
You can find more information in the [`cnpg` plugin page](kubectl-plugin.mdx).
