...
:::info
Please refer to [`VolumeSnapshotConfiguration`](../cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-VolumeSnapshotConfiguration)
...
`plugins` one (controlling the [WAL archive](../wal_archiving.mdx)).
...
:::warning
As noted in the [backup document](../backup.mdx), a cold snapshot explicitly
...
Service Account (IRSA, see [AWS S3](object_stores.mdx#aws-s3)):
