...
CloudNativePG provides support for PostgreSQL tablespaces through _declarative
tablespaces_, operating at two distinct levels:
...
[Bootstrap a new cluster](bootstrap.mdx#bootstrap-an-empty-cluster-initdb) for
details.
...
The [kubectl status](kubectl-plugin.mdx#status) plugin includes a section
