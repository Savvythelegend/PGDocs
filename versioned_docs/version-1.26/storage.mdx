...
Kubernetes has its own specifics. These include _storage classes_, _persistent
volumes_, and _Persistent Volume Claims (PVCs)_. You need to own these
...
to know more, see
[Custom pod controller](controller.mdx).
...
[Bootstrap an empty cluster](bootstrap.mdx#bootstrap-an-empty-cluster-initdb).
...
See [Tablespaces](tablespaces.mdx) for details.
...
3. Create the Postgres `Cluster` using, for each storage section, a coherent
   [`pvcTemplate`](storage.mdx#configuration-via-a-pvc-template)
...
to them, as explained in ["Synchronizing the state"](architecture.mdx#synchronizing-the-state).
...
the broader _shared-nothing architecture_ principle.
...
Additionally, your Postgres `Cluster` should have [pod anti-affinity rules](scheduling.mdx#isolating-postgresql-workloads)
...
replica. If a host becomes corrupted, you can use the [`cnpg` plugin to destroy](kubectl-plugin.mdx#destroy)
