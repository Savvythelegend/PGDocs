...
[probe API documentation](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-ProbeWithStrategy).
...
:::info
For more details on probe configuration, refer to the
[probe API documentation](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-Probe).
...
[probe API](cloudnative-pg.v1.mdx#postgresql-cnpg-io-v1-ProbeWithStrategy).
...
the database [RPO](before_you_start.mdx#rpo), don't delete the Pod where
...
:::warning
The `.spec.switchoverDelay` option affects the [RPO](before_you_start.mdx#rpo)
and [RTO](before_you_start.mdx#rto) of your PostgreSQL database. Setting it to
...
Please refer to the ["Failover" section](failover.mdx) for details.
...
["Monitoring" section](monitoring.mdx#predefined-set-of-metrics)
...
See also the ["Volume expansion" section](storage.mdx#volume-expansion) of the
documentation.
